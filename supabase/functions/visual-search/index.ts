...
serve(async (req) => {
  ...
  try {
    ...
    if (
      ...
      return new Response(
        JSON.stringify({
          error: "Invalid image URL. Must be https and under 2048 characters.",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
...
Do not include any other text, just the JSON.`,
    ...
    if (!productInfo) {
      return new Response(
        JSON.stringify({
          error:
            "Could not identify the product in the image. Please try a clearer image.",
        }),
        {
          status: 422,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (searchResponse && searchResponse.ok) {
      ...
      products = items.map((item: any, index: number) => {
        ...
        const similarity = Math.min(
          0.99,
          0.6 + (matchedWords.length / queryWords.length) * 0.35,
        );

        return {
          id: item.product_id || item.id || `product-${index}`,
          name:
            item.product_title || item.title || item.name || "Unknown Product",
          price,
          originalPrice,
          discount: item.offer?.discount || item.discount || 0,
          rating:
            parseFloat(item.product_rating) || parseFloat(item.rating) || 4.0,
          reviews:
            parseInt(item.product_num_reviews) || parseInt(item.reviews) || 0,
          store:
      ...
      products.sort(
        (a: any, b: any) => b.similarity - a.similarity || a.price - b.price,
      );
    } else {
