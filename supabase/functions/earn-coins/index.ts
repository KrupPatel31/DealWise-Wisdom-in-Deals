...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      case "daily_login":
        return await handleDailyLogin(adminClient, user.id);
      case "claim_referral_code":
        return await handleClaimReferralCode(adminClient, user.id);
      case "use_referral":
        return await handleUseReferral(adminClient, user.id, payload.code);
      case "submit_review":
        return await handleSubmitReview(adminClient, user.id, payload);
      default:
...
async function handleUseReferral(
  adminClient: any,
  userId: string,
  code: string,
) {
  ...
  await awardCoins(
    ...
    userId,
    25,
    "Welcome bonus: used referral code",
  );
...
async function handleSubmitReview(
  ...
  if (!product_id || !product_name || !rating || rating < 1 || rating > 5) {
    return new Response(
      JSON.stringify({
        error: "Valid product_id, product_name, and rating (1-5) required",
      }),
      {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
  ...
  const hasOrdered = orders?.some((order: any) => {
    ...
    return (
      ...
      items.some(
        (item: any) =>
          item.product_id === safeProductId || item.id === safeProductId,
      )
    );
  });
  ...
  return new Response(
    JSON.stringify({
      success: true,
      coins_awarded: 20,
      message: "Review submitted! You earned 20 coins!",
    }),
    {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    },
