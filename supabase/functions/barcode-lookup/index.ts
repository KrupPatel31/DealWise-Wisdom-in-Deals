...
async function lookupWithRapidAPI(
  barcode: string,
  _unused?: string,
): Promise<any | null> {
  try {
    ...
    if (data?.data?.length > 0) {
      const products = data.data.slice(0, 10).map((p: any, i: number) => {
        const price = p.offer?.price
          ? parseFloat(p.offer.price.replace(/[^0-9.]/g, ""))
          : 0;
        const originalPrice = p.offer?.original_price
          ? parseFloat(p.offer.original_price.replace(/[^0-9.]/g, ""))
          : price;
        const discount =
          originalPrice > price
        ...
        products: [
          {
            ...
            category:
              p.categories?.split(",")[0]?.trim() || "Beauty & Personal Care",
            description: (p.ingredients_text || "").substring(0, 200),
            image: p.image_url || p.image_front_url || null,
...
async function lookupWithOpenPetFoodFacts(
  barcode: string,
): Promise<any | null> {
  try {
    const response = await fetch(
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const [rapidResult, offResult, upcResult, beautyResult, petResult] =
      await Promise.all([
        rapidApiKey
          ? lookupWithRapidAPI(cleanBarcode, rapidApiKey)
    ...
    const result =
      rapidResult || offResult || upcResult || beautyResult || petResult;

    // If free DB found info but no prices, try enriching with RapidAPI search by product name
    if (
      result &&
      !rapidResult &&
      result.products?.some((p: any) => p.price === 0)
    ) {
      if (rapidApiKey && result.productName) {
        const searchResult = await lookupWithRapidAPI(
          result.productName,
          rapidApiKey,
        );
        if (searchResult?.products?.length > 0) {
          ...
          const enrichedProducts = [
            // Original product with metadata from free DB
            {
              ...
              originalPrice:
                searchResult.products[0]?.originalPrice ||
                freeProduct.originalPrice,
              store:
                freeProduct.store !== "Unknown"
                  ? freeProduct.store
                  : searchResult.products[0]?.store || "Unknown",
              discount:
                searchResult.products[0]?.discount || freeProduct.discount,
            },
